...
CREATE TABLE IF NOT EXISTS logs.logs_info(
...
CREATE OR REPLACE FUNCTION logs.prc_ins_logs_info(
...
$BODY$
BEGIN
INSERT INTO logs.logs_info("_timestamp","_level","_user_name","_process_name","_pid","_message")
...
COMMENT ON FUNCTION logs.prc_ins_logs_info(timestamp,character varying,character varying,character varying,integer,character varying) IS 'Вставляет данные данные о новой записи';

CREATE TYPE logs.type_logs_info AS (
	_id integer,
...
GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE logs.logs_info to server_role WITH GRANT OPTION;
GRANT SELECT ON TABLE logs.logs_info to client_role WITH GRANT OPTION;
GRANT TEMP ON DATABASE syslog_ng TO client_role WITH GRANT OPTION;

REVOKE ALL ON FUNCTION logs.prc_ins_logs_info(timestamp,character varying,character varying,character varying,integer,character varying) FROM public;
GRANT EXECUTE ON FUNCTION logs.prc_ins_logs_info(timestamp,character varying,character varying,character varying,integer,character varying) TO server_role WITH GRANT OPTION;


GRANT SELECT,UPDATE ON SEQUENCE logs.logs_info__id_seq TO server_role WITH GRANT OPTION;

GRANT server_role to log_writer;
GRANT client_role to log_reader;

CREATE OR REPLACE FUNCTION logs.prc_get_logs_info_with_filter(
...
RETURNS TABLE(log logs.type_logs_info) AS
$BODY$
BEGIN
CREATE TEMP TABLE res_table OF logs.type_logs_info ON COMMIT DROP;
INSERT INTO res_table SELECT * FROM logs.logs_info;
...
COMMENT ON FUNCTION logs.prc_get_logs_info_with_filter(
	timestamp,
	timestamp,
	character varying,
	character varying,
...
CREATE OR REPLACE FUNCTION logs.prc_get_n_filtered_logs_ordered_by_time(
	asc_order boolean,
...
COMMENT ON FUNCTION logs.prc_get_n_filtered_logs_ordered_by_time(
	boolean,
	integer,
	timestamp,
	timestamp,
...
CREATE OR REPLACE FUNCTION logs.prc_get_stat_table_of_warnings(
	f_bot_timestamp timestamp,
	f_ceil_timestamp timestamp,
	f_user_name character varying(255),
	f_process_name character varying(255))
...
COMMENT ON FUNCTION logs.prc_get_stat_table_of_warnings(
	timestamp,
	timestamp,
	character varying,
...
CREATE OR REPLACE FUNCTION logs.prc_get_n_messages(
	asc_order boolean,
	number_of_logs integer,
	f_bot_timestamp timestamp,
	f_ceil_timestamp timestamp,
